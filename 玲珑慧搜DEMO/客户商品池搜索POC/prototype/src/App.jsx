...
export function App() {
  ...
  // The product pool is a focused shortlist: never render more than 30 rows,
  // even if a remote adapter later returns an unbounded candidate set.
  const results = searchState.items.slice(0, 30);
  ...
  return <div className="app-shell">
    ...
    {screen === "home" ? <main className="home-page">
      ...
      <section className="results-section"><div className="results-head"><div><span>{searchState.loading ? "本地索引" : searchState.fallback ? "模糊推荐" : searchState.supplementCount ? "强匹配 + 相关补充" : "商品池"}</span><strong>{searchState.loading ? "正在生成可检索商品索引…" : query ? `展示 ${results.length} 条商品` : `展示前 ${results.length} 条商品`}</strong></div><button className="filter-button"><FunnelSimple size={17} /> 筛选</button></div>{searchState.parsed.categoryClusters?.length > 0 && <div className="category-note"><Sparkle size={16} weight="fill" /> 已识别 <strong>{searchState.parsed.categoryClusters.map((cluster) => cluster.displayName || cluster.name).join("、")}</strong>，将优先展示同类商品；单字中文不会再用拼音泛匹配跨品类商品。</div>}{searchState.fallback && <div className="fallback-note"><Sparkle size={16} weight="fill" /> 未命中可靠的强匹配，以下按名称、别名、拼音、型号片段与字符相似度排序；当前仅展示前 30 条。</div>}{!searchState.fallback && searchState.supplementCount > 0 && <div className="supplement-note"><Sparkle size={16} weight="fill" /> 已优先展示强匹配；其余位置以低置信度相关商品补充，当前仅展示前 30 条。</div>}{searchState.loading ? <div className="index-loading"><Sparkle size={24} weight="fill" /><strong>正在为 {products.length} 条商品建立本地索引</strong><span>索引完成后即可使用精确、容错与模糊检索。</span></div> : results.length ? <div className="product-table" role="table"><div className="table-head" role="row"><span>商品</span><span>品牌</span><span>型号 / 规格</span><span>匹配方式</span><span>操作</span></div>{results.map(({ product, reason, confidence }) => <div className="table-row" role="row" key={product.id}><div className="product-cell"><ProductImage src={product.image} name={product.name} /><span><strong>{product.name}</strong><small>{product.model}</small></span></div><span>{product.brand}</span><span>{product.model}</span><span><i className={`match-tag ${confidence === "高" ? "exact" : ""} ${confidence === "低" ? "weak" : ""}`}>{reason}</i></span><button className="detail-link" onClick={() => setDetail(product)}>查看详情 <ArrowRight size={15} /></button></div>)}</div> : <div className="empty-results"><MagnifyingGlass size={36} weight="duotone" /><h2>商品池为空</h2><p>请先上传商品材料，系统会自动识别可检索字段。</p><button className="subtle-button" onClick={() => setScreen("home")}>返回上传</button></div>}</section>
    </main>}
